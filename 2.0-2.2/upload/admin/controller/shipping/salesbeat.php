...
class ControllerShippingSalesbeat extends Controller
{
    private $error = [];
    ...
    public function index()
    {
        ...
        $data['breadcrumbs'][] = [
            ...
            'href' => $this->url->link('shipping/salesbeat', 'token=' . $this->session->data['token'], 'SSL')
        ];

        // Links
        $data['action'] = $this->url->link('shipping/salesbeat', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $this->response->setOutput($this->load->view('shipping/salesbeat.tpl', $data));
    }

    protected function validate()
    {
        if (!$this->user->hasPermission('modify', 'shipping/salesbeat'))
    ...
    {
        $this->load->model('shipping/salesbeat');
        $this->model_shipping_salesbeat->install();
    }

    public function uninstall()
    {
        $this->load->model('shipping/salesbeat');
        $this->model_shipping_salesbeat->uninstall();
    }
}