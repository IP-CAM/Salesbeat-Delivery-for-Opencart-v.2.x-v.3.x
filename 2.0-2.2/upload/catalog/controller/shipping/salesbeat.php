...
class ControllerShippingSalesbeat extends Controller
{
    ...
    public function index()
    {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/shipping/salesbeat.tpl')) {
            $template = $this->config->get('config_template') . '/template/shipping/salesbeat.tpl';
        } else {
            $template = 'shipping/salesbeat.tpl';
        }
    ...
    {
        $sbSetting = [];
        $sbSetting['url'] = $this->url->link('shipping/salesbeat/save', '', true);
